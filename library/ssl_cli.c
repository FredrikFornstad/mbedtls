                                    ...
                                    size_t *olen )
{
    ...
    *olen = 0;
    ...
    p = poff + ssl->out.buf;
    end = ssl->out.msg + ssl->out.max_content_len;
...
{
    int ret;
    size_t i, n, olen, ext_len = 0, poff;
    unsigned char *buf;
    unsigned char *p, *q;
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
    ret = mbedtls_ssl_confirm_content_len( ssl, &ssl->out, poff + 2 + ext_len +
...
static int ssl_write_certificate_verify( mbedtls_ssl_context *ssl )
{
    ...
    ret = mbedtls_ssl_confirm_content_len( ssl, &ssl->out,
                                               (size_t)( ssl->out.msg - ssl->out.buf + offset
                                                + 6 + mbedtls_pk_get_len( mbedtls_ssl_own_key( ssl ) ) ) );
    if( ret )
        return( ret );
